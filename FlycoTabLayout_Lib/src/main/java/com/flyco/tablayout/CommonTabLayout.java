...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    public void showMsg(int position, int num) {
        ...
        if (tipView != null) {
            ...
            if (!mIconVisible) {
                setMsgMargin(tipView,position, 2, 2);
            } else {
                setMsgMargin(tipView,position, 2,
     ...
     */
    public void setMsgMargin(MsgView tipView,int position, float leftPadding, float bottomPadding) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        View tabView = mTabsContainer.getChildAt(position);
        if (tipView != null) {
            ...
            float tipHeight = tipView.getLayoutParams().height;
...
//                lp.topMargin = mHeight > 0 ? (int) (mHeight - textHeight - iconH - margin) / 2 - dp2px(bottomPadding) : dp2px(bottomPadding);
                lp.topMargin = (int) (margin - tipHeight/2);
            } else {
                lp.leftMargin = dp2px(leftPadding);
